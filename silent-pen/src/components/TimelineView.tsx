...
const TimelineView: React.FC<TimelineViewProps> = ({ diaryList }) => {
  return (
    <div style={{ marginTop: 32, paddingLeft: 32 }}>
      ...
      <div style={{ borderLeft: '3px dashed #b0b8c1', paddingLeft: 0 }}>
        {diaryList.length === 0 && <div style={{ color: '#888', paddingLeft: 40 }}>暂无封存日记</div>}
        {diaryList.map((item, idx) => (
          <div key={item.date + item.time + idx} style={{ marginBottom: 22, display: 'flex', alignItems: 'center', paddingLeft: 18 }}>
            <div style={{
              width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center',
              background: '#fff', borderRadius: '50%', boxShadow: '0 0 0 2px #b0b8c1', zIndex: 1, marginRight: 18
            }}>
              <LockIcon />
            </div>
